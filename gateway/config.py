...
def _coerce_bounded_int(
    ...
    """Coerce integer config values within inclusive bounds."""
    if value is None:
        return default
    if isinstance(value, bool):
        logger.warning(
            "Ignoring invalid %s=%r (expected integer in %s..%s)",
            key,
            value,
            min_value,
            max_value,
    ...
    if parsed < min_value or parsed > max_value:
        logger.warning(
            "Ignoring invalid %s=%r (must be between %s and %s)",
            key,
            value,
            min_value,
            max_value,
...
def _coerce_file_path(value: Any, key: str) -> str:
    raw = _coerce_nonempty_str(value)
    if not raw:
        return ""
    if not Path(raw).is_absolute():
        logger.warning("Ignoring invalid %s=%r (expected absolute path)", key, value)
        return ""
    return raw
...
class GPTLiveConfig:
    ...
    cooldown_max_seconds: int = 3_600
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GPTLiveConfig":
        ...
        model = _coerce_nonempty_str(data.get("model")) or defaults.model
        protocol_version = _coerce_nonempty_str(data.get("protocol_version")) or defaults.protocol_version
        if protocol_version not in {"v2", "v3"}:
            protocol_version = defaults.protocol_version

        voice = _coerce_nonempty_str(data.get("voice")) or defaults.voice
        mode = _coerce_nonempty_str(data.get("mode")) or defaults.mode
        ...
        if isinstance(allowlist_raw, list):
            allowlist = [str(item).strip() for item in allowlist_raw if isinstance(item, str)]
        ...
        return cls(
            ...
            voice=voice,
            mode=mode,
            codex_binary=_coerce_file_path(data.get("codex_binary"), "gpt_live.codex_binary"),
            devicecheck_addon=_coerce_file_path(
                data.get("devicecheck_addon"),
                "gpt_live.devicecheck_addon",
            ),
...
def load_gateway_config() -> GatewayConfig:
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            elif isinstance(gateway_section, dict) and "gpt_live" in gateway_section:
                gw_data["gpt_live"] = gateway_section["gpt_live"]
