...
async def auth_native_authorize(
    ...
    if p is None and not provider:
        native_eligible = [
            ...
            if getattr(pp, "supports_interactive_login", True)
            and not getattr(pp, "supports_password", False)
        ]
        if len(native_eligible) == 1:
            ...
            if len(sess_providers) == 1 and getattr(
                sess_providers[0], "supports_password", False
            ):
                p = sess_providers[0]
    if p is None:
        raise HTTPException(
